...
\subsection*{Graph-augmented fine-mapping (GAFM)}\label{methods:l1}
...
1KG NYGC GRCh38 for human). For the rice grain weight + shape pass
(this work, v0.1.4), a 5-method panel was applied at the top 5
genome-wide-significant + top 2 suggestive leads per trait
(28 loci total) within $\pm$100\,kb LD windows after filtering to
...
in the v0.1.4 release. The public SBayesRC LD reference is
human HM3-EUR (wrong species), so a 3kRG-specific LD reference
was constructed from scratch using gctb 2.05beta and the R
...
\textbf{v0.1.5 GAFM/HBP enhancements.} The v0.1.4 5-method panel above
revealed that GAFM and HBP, while 200--700$\times$ faster than SuSiE on
this rice grain pass, gave large credible sets (median CS $\approx$ 1{,}500)
on secondary loci under the strong inflation regime
($\lambda_{GC}{=}1.41$--$1.78$ from XI/GJ structure under PC1--PC10).
SBayesRC's continuous-shrinkage mixture prior dominated those secondary
loci, suggesting that the same effect-size-distribution correction would
help GAFM/HBP. v0.1.5 adds three corrections, all applied by default:
...
correction and is now the default for all six original methods plus
the three new ones below.
\emph{(ii)} \textbf{Mixture-prior posterior reweighting (MX)}: the
per-variant PIPs from GAFM and HBP are multiplied by a SBayesRC-style
4-component Wakefield mixture-BF on the deflated z-scores
($\pi=(0.005, 0.003, 0.001, 0.001)$ renormalised within the non-zero
components; $\gamma=(0.001, 0.01, 0.1, 1.0)$; $V_k = N\gamma_k$;
ABF$_k(z) = (1+V_k)^{-1/2} \exp(z^2 V_k / (2(1+V_k)))$). The reweighted
distribution preserves $\sum_i \mathrm{PIP}_i$ (expected number of causals
unchanged) but redistributes mass toward large-$|z|$ variants. The two
new methods are written GAFM-MX and HBP-MX.
...
at 95\% coverage. The combined v0.1.5 panel is therefore nine methods:
GAFM, HBP, GAFM-MX, HBP-MX, ENS, SuSiE, SuSiE-inf, FINEMAP-inf,
SBayesRC; the curated five-method best-of-class subset
(GAFM-MX, HBP-MX, ENS, SuSiE, SBayesRC) is shown in
Supplementary Table~S8 and the full nine-method comparison is in
\texttt{data/rice\_3k/results/grain\_finemap/grain\_finemap\_summary.tsv}. Validation used 250~kb-window overlap
...
HBP and GAFM; CLGF (cross-locus EM) and L4 (graph-neural-network
