...
\texttt{tabix}\cite{li2011tabix} over HTTPS against the public S3 bucket
...
BGEN; Pan-UKB in-sample LD BlockMatrices (Hail\cite{hailteam2024} format, 47.6~TB
...
after, and per-variant $z$-statistics agree with PLINK2\cite{chang2015second} logistic on
...
The default $\alpha$ is benchmark-specific and is chosen once per
benchmark from the v0.1.5 driver scripts in \texttt{tests/}:
$\alpha{=}0.7$ for the strong-signal six-method head-to-head
benchmark (\texttt{benchmark\_v15\_chr22.py}; Sup Table~S2), the
v0.1.5 PIP calibration and null FPR experiments
(\texttt{benchmark\_v15\_calibration\_null.py}), the cross-species
replication on yeast / \emph{Arabidopsis} / IRRI 3kRG, and the
3kRG grain weight + shape pass
(\texttt{rice3k\_grain\_shape\_finemap.py}); $\alpha{=}0.5$ for
the weak-signal tissue-specific eQTL regime
(\texttt{benchmark\_weak\_signal.py}; the 27\,--\,2 GAFM-vs-SuSiE
result and its 100-rep replication); and $\alpha{=}1.0$ for the
sumstats-only Pan-UK Biobank fine-mapping path
(\texttt{benchmark\_panukb\_finemap.py}), where no eQTL
functional prior is available at runtime. A hyperparameter
sensitivity sweep across $\alpha \in \{0.3, 0.5, 0.7, 0.9\}$
confirms that GAFM rank-\#1 rate degrades sharply for
$\alpha < 0.6$ and saturates from $\alpha{=}0.7$ upward, motivating
the chosen 0.7 default whenever graph priors carry the headline
(sweep reported in Sup Note~S2). PIPs are the softmax:
...
GCTB\cite{zeng2018gctb}~2.05beta and the R
...
GraphGWAS platform. Paper-facing names map to Python module
identifiers as follows: \textbf{GAFM} $\rightarrow$
\texttt{l1\_finemap\_from\_sumstats} (and CLI
\texttt{graphgwas finemap-sumstats --method gafm});
\textbf{HBP} $\rightarrow$ \texttt{hbp\_finemap\_from\_sumstats};
\textbf{GAFM-MX} / \textbf{HBP-MX} $\rightarrow$
\texttt{gafm\_mx\_from\_sumstats} / \texttt{hbp\_mx\_from\_sumstats};
\textbf{ENS} $\rightarrow$ \texttt{ensemble\_from\_sumstats};
\textbf{CLGF} $\rightarrow$ \texttt{cross\_locus\_graph\_finemap};
\textbf{LPCE} (epistasis preview) $\rightarrow$ \texttt{lpce\_*}
in \texttt{graphgwas.epistasis\_v2}. The \texttt{l1\_*} and
\texttt{m1\_*} prefixes are historical from the platform's
internal layer-numbering and are retained in module names for
back-compatibility; the paper-facing names are canonical.
...
53-command CLI hierarchy is visualised as Supplementary
Figure~S3; a per-command reference manual is shipped at
\texttt{docs/manual/index.md}, an end-to-end vignette
reproducing the FTO/BMI headline result from Pan-UKB summary
statistics in $\sim$15 minutes is provided at
\texttt{vignettes/fine-mapping-quickstart.md}, and a
practitioner's guide on preparing inputs (z-score $\lambda_{GC}$
and LD-matrix sanity checks; required column formats), choosing
the appropriate method, and interpreting the credible-set output
(including the calibrated-vs-operational distinction between
base GAFM/HBP and the mixture-prior reweightings) is at
\texttt{docs/INPUT\_OUTPUT\_GUIDE.md}. The same procedures
