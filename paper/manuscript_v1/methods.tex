...
Evaluation System traits).} Whole-genome single-locus association
was performed with PLINK~2.0 \cite{chang2015second}. The
\emph{Arabidopsis} pipeline ran
...
biallelic SNPs from the 3kRG pseudo-canonical VCF) ran
\texttt{plink2 --bfile rice\_3krg --maf 0.01 --geno 0.05 --pheno
irri\_18traits.tsv --linear hide-covar --covar
rice\_10pcs.tsv --out rice\_\{trait\}} for each of the 18 IRRI
trait codes. Phenotype files were prepared by the
\texttt{graphgwas.phenotype} module, which links accession IDs
across heterogeneous source identifiers (IRGC accession numbers
to PLINK sample IDs for rice). Genotypic principal components
were computed with \texttt{plink2 --pca 10}.
