...
\centering
\small
\caption{\textbf{Adding a per-variant heterogeneous prior to the
multi-omics cache turns HBP from inert to informative across four
species.} Adding a per-variant continuous prior to the
...
narrows the credible set beyond flat-prior GAFM. The human row is the
cleanest demonstration: a uniform GTEx + STRING cache leaves HBP
exactly equal to GAFM on every one of 321 leads, but adding a
