...
(dashed). SuSiE, FINEMAP and L1 track the diagonal; HBP is
...
50~kb yeast loci. Mean $\max$\,PIP is 0.003 (HBP), 0.004 (L1), 0.013
...
5\times10^{-8}$. ``CS$=$1 (L1)'' is the count among the 4
fine-mapped leads per trait for which L1 narrows the 95\% credible
...
\begin{tabular}{lrrrrr}
\toprule
\textbf{Trait code} & \textbf{N} & $\lambda_{GC}$ & \textbf{min}~$p$ & \textbf{GW indep.} & \textbf{CS$=$1 (L1)} \\
\midrule
...
L1 CS $=$ 1 is the fraction of leads where flat-prior Bayesian L1
produces a single-variant 95\% credible set. ``HBP tighter'' is the
fraction of leads where HBP narrows the credible set further than L1
