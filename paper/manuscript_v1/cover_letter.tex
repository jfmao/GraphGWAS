...
CSA, AFR, EAS), recovering \emph{LDLR}, \emph{APOE}, \emph{LPL}, \emph{GCKR} and \emph{ANGPTL3}
at single-variant resolution.
...
Genomes panel (14 traits, 54 leads) and the 3{,}000 Rice Genomes panel (18 IRRI traits, 72
leads); on rice, 24 of 72 fine-mapped leads fall within 250\,kb of a Ren \emph{et al.}\ (2023)
catalogued causal gene, including \emph{Rc} (4.5\,kb) and \emph{TGW6} (12.3\,kb).
\end{itemize}

A direct-intervention experiment establishes the central methodological claim. Adding a coarse
nine-class ENCODE regulatory-element flag to an otherwise uniform human cache flips the rate at
which HBP narrows the credible set tighter than GAFM from \textbf{0\% to 88\%} on the same 321
human Pan-UKB leads, with no other change to GWAS sumstats, LD references, or the fine-mapping
algorithm. Across all 692 leads in the four-species replication, the same intervention raises
the HBP-tighter-than-GAFM rate from 41\% to 92\%. Density alone is therefore insufficient even
at $10^7$-variant scale; the operative quantity is per-variant discriminative information,
which the typed-graph representation exposes directly to inference.
...
species --- crops, livestock, and model organisms. The 3{,}000 Rice Genomes recovery of \emph{Rc}, \emph{TGW6}, \emph{An-1},
\emph{BG1}, \emph{GAD1/RAE2} and \emph{OsER1} from the same code base that produces
single-variant credible sets at \emph{LDLR} and \emph{LPL} on Pan-UK Biobank is a concrete
demonstration of this portability on the canonical crop-genomics benchmark species.
