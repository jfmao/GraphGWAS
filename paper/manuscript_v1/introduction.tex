...
accuracy--speed frontier. GraphGWAS is a fine-mapping platform
built on a graph database (Neo4j~5.26 Community Edition is the
present implementation\cite{robinson2015graph}) holding 70.7~M
variants, 20{,}092 GENCODE~v47 genes\cite{frankish2023gencode},
43.2~M GTEx~v8 eQTL edges, 230{,}850 STRING interactions and
370{,}000 ENCODE regulatory elements. Its central methods ---
hierarchical belief propagation (HBP), with proved geometric
convergence via a Banach contraction argument, and L1 Bayesian
fine-mapping, with a proof that the causal variant ranks first
under mild LD-decay assumptions --- carry the full
...
loci, HBP matches SuSiE and FINEMAP at 6--60$\times$ the speed; on
79 weak-signal tissue-specific eQTL simulations, L1 beats SuSiE
27--2 head-to-head (sign-test $p < 10^{-5}$). PIPs are calibrated
with 0\% null false-positive rate across 100 simulations. A
...
narrower than flat-prior L1 from 0\% to 88\% of leads.
