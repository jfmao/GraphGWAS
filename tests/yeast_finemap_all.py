...
def finemap_locus(trait: str, lead: dict, sumstats: pd.DataFrame,
    ...
    n_samples = 1011  # 1011 Yeast Genomes panel default
    ...
    gx_top = sorted(gafm_mx, key=lambda c: -c.pip)[:3]
    hx_top = sorted(hbp_mx, key=lambda c: -c.pip)[:3]
    en_top = sorted(ens, key=lambda c: -c.pip)[:3]
    return {
        ...
        "gafm_mx_top_variant": gx_top[0].variant_id if gx_top else None,
        "gafm_mx_top_pip": gx_top[0].pip if gx_top else None,
        "hbp_mx_top_variant": hx_top[0].variant_id if hx_top else None,
        "hbp_mx_top_pip": hx_top[0].pip if hx_top else None,
        "ens_top_variant": en_top[0].variant_id if en_top else None,
        "ens_top_pip": en_top[0].pip if en_top else None,
    }
